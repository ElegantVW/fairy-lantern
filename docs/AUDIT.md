...
| 24 | Almost no regression tests | **Partial** — 107 unit tests; still no PPU goldens / committed PCM fixture |
...
- ROM 16-bit access latches open bus as `half | half<<16` (FRLG `DestroySprite(NULL)`).
- IWRAM/EWRAM 16-bit reads latch the aligned 32-bit word; VRAM/PAL/OAM duplicate the halfword.
- ObjAffineSet / BgAffineSet use a 256-step 8.8 sine table (LC calls 0x0F constantly).
